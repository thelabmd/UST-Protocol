...
| `ust sign <data.json> --key <f> --ust-id <ust:…>` | turn YOUR data into a signed transcript. Privacy is declared PER PARTITION in the data, exactly as the wire carries it — `"privacy":"blinded"` or `"encrypted"` beside `kind`. A private partition makes `--disclosures-out` **mandatory**: the nonce is generated here, so a tool that keeps it would leave you holding a commitment you can never open. Add `--aead-keys` for `encrypted` — this tool does not invent a key you cannot rotate (spec §10 leaves key management to you — `§` throughout this file means a section of [`spec/UST-1.0.md`](https://github.com/thelabmd/UST-Protocol/blob/main/spec/UST-1.0.md)). |
