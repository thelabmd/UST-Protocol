...
export function buildWorkerScript(genesisText) {
  ...
  // The genesis is already IN the worker; a cache saved nothing (the invocation happens either way) and
...
export default {
  async fetch(req) {
    ...
    const res = new Response(req.method === 'HEAD' ? null : GENESIS, { headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'public, max-age=300' } });
    return res;
  }
};
`;
