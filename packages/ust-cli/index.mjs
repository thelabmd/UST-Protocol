...
export function buildWorkerScript(genesisText, keylogText = null) {
  ...
  // APPEND + redeploy — the log only ever GROWS; it is never rewritten.
...
export function buildWranglerProject({ domain, genesisText, keylogText = null }) {
  return {
    'worker.mjs': buildWorkerScript(genesisText, keylogText),
    'wrangler.toml': [
      `name = "ust-genesis-${domain.replaceAll('.', '-')}"`,
      'main = "worker.mjs"',
      'compatibility_date = "2026-01-01"',
      'workers_dev = false',
      `routes = [{ pattern = "${domain}/.well-known/ust-genesis*", zone_name = "${domain}" }${keylogText !== null ? `, { pattern = "${domain}/.well-known/ust-keylog*", zone_name = "${domain}" }` : ''}]`,
    ].join('\n') + '\n',
  };
}
...
export async function cfPublish({ domain, genesisText, keylogText = null, token, flipProxy = false, fetchImpl = fetch }) {
  ...
  form.append('worker.mjs', new Blob([buildWorkerScript(genesisText, keylogText)], { type: 'application/javascript+module' }), 'worker.mjs');
...
async function cmdPublish() {
  ...
  if (arg('auth', null) === 'wrangler') {
    ...
    let w; try { w = await wranglerDeploy({ domain, genesisText, keylogText }); } catch (e) { die(e.message); }
    ...
    try { r = await cfPublish({ domain, genesisText, keylogText, token, flipProxy }); } catch (e) { die(e.message); }
  ...
  if (publishMode === 'cf') {
    ...
    try {
      if (authMode === 'wrangler') {
        ...
        const w = await wranglerDeploy({ domain, genesisText: JSON.stringify(genesis), keylogText: JSON.stringify([keylog0]) });
        ...
        pub = await cfPublish({ domain, genesisText: JSON.stringify(genesis), keylogText: JSON.stringify([keylog0]), token: process.env.CF_TOKEN || process.env.CLOUDFLARE_API_TOKEN, flipProxy: !!arg('flip-proxy', false) });
      }
    } catch (e) { rl.close(); die(e.message); }
