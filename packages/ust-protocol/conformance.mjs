...
//     key loss, WITHOUT bypassing checkpoint validation. Dormant emergency mechanism, not a normal rotation.
{
  ...
  { const src = readFileSync(new URL('../../packages/ust-protocol/index.mjs', import.meta.url), 'utf8');
    const SELECTORS = ['keylog', 'genesis', 'checkpoint', 'pinnedKeys', 'cadenceLog'];
    const coalesced = SELECTORS.filter((s) => new RegExp('Array\\.isArray\\(' + s + '\\)\\s*\\?\\s*' + s + '\\b').test(src));
    check('R43 STRUCTURAL: no authority SELECTOR uses the `Array.isArray(X) ? X : default` coalesce — a present malformed selector is admitted, never silently emptied' + (coalesced.length ? ' — FOUND: ' + coalesced.join(',') : ''), coalesced.length === 0);
    const REGISTERED = new Set(['requirePerFrameValid', 'allowExperimentalAttested', 'acceptConsumerOverride', 'noForkConfirmed', 'corroborated', 'requireAuthoritative', 'requireAnchored', 'requireFreshKeylog', 'offline', 'requireVersion']);   // requireVersion is a STRING version match (`=== '1.0'`), not a truthy grant — registered so the scan is exhaustive
    ...
    check('R43 STRUCTURAL: every require*/allow*/acceptConsumerOverride security-policy boolean in index.mjs is REGISTERED (a new grant flag fails until admitted via admitBool + adversarially probed)' + (unregistered.length ? ' — UNREGISTERED: ' + unregistered.join(',') : ''), unregistered.length === 0);
