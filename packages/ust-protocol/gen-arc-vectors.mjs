...
// ─── uniqueness units (F.5j/F.5k) — witness quorum + map, both bases; and name-map authoritative ───
{
  ...
  add('uniqueness-extra-field-claim-dropped', 'verifyCheckpointUniqueness', { ...u4([{ claim: { ...P.checkpointUniquenessClaim({ domain_shard: D, genesis_epoch: EPc, sequence: '0', checkpoint: CP }), observed_map_root: 'sha256:' + 'aa'.repeat(32) }, issuer_id: Wa.key_id, sig: ua(Wa).sig }, ua(Wb)]), expect: { attested: false } });   // round-34 P0-03 — a claim with a field ∉ the closed VOTE_CLAIM is DROPPED (uniqueness cannot conflict under the closed claim; rival-quorum determinism lives in the recovery path)
