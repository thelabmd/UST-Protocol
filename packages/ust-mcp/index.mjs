...
export const tools = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', required: ['domain_shard', 'ust_id', 'key_id', 'time', 'key_op', 'prev'], properties: { domain_shard: { type: 'string' }, ust_id: { type: 'string' }, key_id: { type: 'string' }, time: { type: 'object' }, key_op: { type: 'object', description: '{ op:"add", pub, new_key_id?, supersedes?, role? } | { op:"revoke", pub, reason, compromised_since? } — `supersedes` NAMES the key being replaced (never inferred from position); `role` is admissible only if the genesis DECLARES roles' }, prev: { type: 'string', description: 'content_hash of the prior key-log entry, or of the genesis for the first' } } },
