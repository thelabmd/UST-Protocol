...
[![CI](https://img.shields.io/github/actions/workflow/status/thelabmd/UST-Protocol/ci.yml?branch=main&label=CI)](https://github.com/thelabmd/UST-Protocol/actions) [![conformance](https://img.shields.io/badge/conformance-1060%20checks%20%C2%B7%20143%20vectors%20%C2%B7%204007%20fuzz-brightgreen)](https://github.com/thelabmd/UST-Protocol/actions/workflows/ci.yml)
...
<picture>
  ...
  <img alt="UST status: `1.0.0-rc.73` — a release candidate, not a final 1.0. Verify machine-readable state without trusting whoever handed it to you. Multiple external AI reviews incorporated structurally; an independent human cryptographic audit is pending; suitable for evaluation and integration testing. The wire format ust:&quot;1.0&quot; is stable across all release candidates; pin exact versions." src=".github/status-light.svg">
</picture>

> **Status: `1.0.0-rc.73`** — a release candidate, not a final 1.0. External AI reviews are folded in structurally; an independent human cryptographic audit is pending. Suitable for evaluation and integration testing. The wire format `ust:"1.0"` is stable across all rc's — pin exact versions. *(This line mirrors the panel above as plain text, for readers and agents that don't render the image.)*
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset=".github/ust-anatomy.svg">
  <source media="(prefers-color-scheme: light)" srcset=".github/ust-anatomy-light.svg">
  <img alt="Anatomy of a UST transcript — a self-contained signed JSON document. Its id says WHO (domain_shard: a name or a self-certifying key-id) and WHEN (ust_id: an address on one shared UTC axis), plus key_id and class. A time frame carries generated_at, valid_from and valid_to. The data holds partitions (captured, computed, blinded or encrypted); one domain-separated hash per partition binds the data to the id and frame. Provenance (based_on, prev, seed) links chains, streams and derivations. An Ed25519 signature travels WITH the data, not the channel. Seal at creation, store anywhere, verify offline in one call — no blockchain; TLS secures the pipe, UST secures the payload, so it verifies the same from a cache, mirror, file or chat paste." src=".github/ust-anatomy-light.svg">
</picture>
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset=".github/ust-chain.svg">
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset=".github/ust-cli.svg">
  <source media="(prefers-color-scheme: light)" srcset=".github/ust-cli-light.svg">
  <img alt="The ust CLI — one entrypoint, the whole command surface (parsed from the real binary’s help). Install with npm i -g @ust-protocol/cli. The 14 subcommands: verify &lt;file|-&gt; (verify a transcript — exit 0 = VALID, 1 = not (--require-anchored demands proven time)); canon &lt;file|-&gt; (print canonical bytes + hash — diff another language's implementation against this); stream &lt;frames…&gt; (a verdict about a RANGE, not one document: chain · forks · completeness (--checkpoint is what makes completeness answerable at all)); forkchoice &lt;docs…&gt; (pick the CANONICAL document among candidates for ONE ust_id — the anchor decides, never the candidates themselves); discovery &lt;domain&gt; (probe a domain's serving surface and report an honest verdict — any infrastructure); names &lt;dir|file…&gt; (point the NAME rule at YOUR OWN published set: an artifact either IS a document of this protocol or does not wear its name (offline; F.5t)); genesis --domain &lt;d&gt; (run the HIGH genesis ceremony (add --publish cf for one-click serving)); key add --domain &lt;d&gt; --root &lt;enc&gt; --role &lt;data|issuance&gt; (ADD a key BESIDE the current one (never replaces it)); rotate --domain &lt;d&gt; --root &lt;enc&gt; (APPEND a key rotation to the served log (never re-mints — documents signed by the old key stay valid)); cadence --domain &lt;d&gt; --root &lt;enc&gt; --seconds &lt;n&gt; --effective-from &lt;slot&gt; ( DECLARE the signed grid your stream follows — what completeness is measured against); reroot --domain &lt;d&gt; --ca-key &lt;enc&gt; (RE-ROOT onto a new genesis, crossing EVERY genesis-rooted structure you run — key-log, authority chain, witness log (the NAME), cadence. Writes artifacts; publishes nothing. Your writer must cross the last axis itself.); publish &lt;cf|self&gt; --domain &lt;d&gt; --genesis &lt;f&gt; (serve an existing genesis: cf deploys the adapter, self writes the four artifacts for YOUR stack (asked if omitted)); mirror &lt;domain&gt; (publish and attest a SECOND-vendor copy, so your identity does not rest on one provider); witness rekor --domain &lt;d&gt; (log the genesis in a public transparency log, so a second published history cannot go unnoticed). Exit 0 = VALID with the tier in the verdict, 1 = not; the ceremony self-verifies its outputs, fail-closed." src=".github/ust-cli-light.svg">
</picture>
