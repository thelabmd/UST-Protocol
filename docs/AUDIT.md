...
npm     npm i ust-protocol@rc          # the reference verifier + producer (Apache-2.0, zero-dep, node:crypto)
        npx -y @ust-protocol/mcp@rc   # the MCP server (22 tools)
...
- **275 deterministic conformance vectors** plus a behavioral conformance runner (**1083 registered checks**), and the reference impl passes them (one known note: duplicate-key
...
- **270 recorded rounds** in `CHANGELOG.md`, carrying the reference-checker from `rev3` to `rev94`. The adversarial
