...
npm     npm i ust-protocol@rc          # the reference verifier + producer (Apache-2.0, zero-dep, node:crypto)
        npx -y @ust-protocol/mcp@rc   # the MCP server (29 tools)
...
- **273 recorded rounds** in `CHANGELOG.md`, carrying the reference-checker from `rev3` to `rev94`. The adversarial
