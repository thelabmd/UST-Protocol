...
const CAPS = {
  ...
  'checkpoint-chain':   { core: ['buildAuthorityCheckpoint', 'sealAuthorityCheckpoint', 'authorityCheckpointId', 'verifyAuthorityCheckpointChain', 'resolveCheckpointRoots', 'deriveCheckpointFreshness'], cli: 'buildCeremony' },
...
const SURFACES = {
  ...
  'ust-mcp':          { probe: mcpProbe, full: ['canon', 'content-address', 'build-transcript', 'verify', 'resolve-authority', 'no-fork-evidence', 'consumer-trust-root', 'anchor-verify', 'fork-choice', 'stream-verify'], subset: [], naReason: 'agent-facing surface — operator ceremonies (checkpoint/recovery/epoch/uniqueness/map/keylog-commitment) stay core+CLI by owner decision', naSpecific: { 'sign': 'the agent signs with its OWN key; build tools return signing_input, the MCP never holds a private key' } },
