...
// AUDIT #114, pass 2 — FLOORS. Every number this battery reports is a RATIO over two sets, and a ratio over an empty
// set is green for free: measured, emptying `conformance-checks.json` made this gate pass, because 0 unproven is ≤ any
// pin. In CI the manifest is regenerated by the conformance step first, so there was no live hole — but a step must
// carry its own claim, and this one could not. The floors are measured, not chosen.
if (suiteIds.length < 500) { console.error(`  ✗ the executed manifest holds ${suiteIds.length} check(s) — under the floor of 500, so every ratio below is computed over a set that is not there`); process.exit(1); }
if (REG.length < 50) { console.error(`  ✗ the lockstep registry holds ${REG.length} record(s) — under the floor of 50, so "N of M proven" is a ratio over nothing`); process.exit(1); }
if (MUTANTS.length < 10) { console.error(`  ✗ only ${MUTANTS.length} mutation(s) carry an observe channel — under the floor of 10, so this battery attacks almost nothing`); process.exit(1); }
